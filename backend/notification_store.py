...
def get_notifications(
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Get unread count
            unread_query = f"SELECT COUNT(*) FROM {DB_SCHEMA}.notifications WHERE user_id = %s AND is_archived = FALSE AND is_read = FALSE"
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                INSERT INTO {DB_SCHEMA}.notification_preferences (
                    ...
                    COALESCE({json.dumps(types_config) if types_config else "'{}'"},
